const DAPP_PORT = process.env.DAPP_PORT || 7777
const DAPP_HOST = process.env.DAPP_HOST || "127.0.0.1"
...
module.exports = function (grunt) {
	...
	grunt.registerTask("bundle", "", function () {
		...
		const webOnlyModules = ["node_modules/ledger-wallet-provider/dist/ledgerwallet.js"]
		...
		for (let x in standaloneLibs) {
			...
			fs.writeFileSync(
				jsStandaloneDirPath + "/" + standaloneLibs[x].name,
				jsStandaloneFileContent,
				"utf8"
			)
		}
		...
		for (let x in standaloneFonts) {
			...
			fs.copyFileSync(
				standaloneFonts[x].path,
				fontsStandaloneDirPath + "/" + standaloneFonts[x].name
			)
		}
	})
}
