	...
	angular
		...
		.service("Transaction", function (Web3Service, Wallet, $rootScope, $q) {
			...
			factory.add = function (tx) {
				...
				Web3Service.web3.eth.getTransaction(tx.txHash, getTransactionInfo)
			}
			...
			factory.simulate = function (tx, cb) {
				...
				Web3Service.sendTransaction(Web3Service.web3.eth, [tx], options, function (e, txHash) {
					if (e) {
						cb(e)
					} else {
						cb(null, txHash)
			...
			factory.simulateMethod = function (tx, abi, method, params, cb) {
				...
				try {
					Web3Service.sendTransaction(instance[method], params, options, function (e, txHash) {
						if (e) {
							cb(e)
						} else {
							cb(null, txHash)
			...
			factory.sendRawTransaction = function (tx, cb) {
				Web3Service.web3.eth.sendRawTransaction(tx, cb)
			}
			...
			factory.checkReceipts = function () {
				...
				for (var i = 0; i < txHashes.length; i++) {
					...
					if (tx && !tx.info) {
						batch.add(Web3Service.web3.eth.getTransaction.request(txHashes[i], getTransactionInfo))
					}
				}
			...
			factory.getEthereumChain = function () {
				return $q(function (resolve, reject) {
					Web3Service.web3.eth.getBlock(0, function (e, block) {
						...
						data.chain = "CIP"
						data.etherscan = "https://explorer.cipherem.com/"
