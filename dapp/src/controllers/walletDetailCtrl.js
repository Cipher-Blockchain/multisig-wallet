		...
		.controller(
			...
			function (
				...
				$scope.$watch(
					...
					function () {
						...
						tokenAddresses.map(function (item) {
							...
							if (
								$scope.wallet.tokens &&
								$scope.wallet.tokens[item] &&
								walletCopy.tokens &&
								walletCopy.tokens[item]
							) {
				...
				$scope.showSafeMigrationModal = function () {
					$uibModal.open({
						...
						controller: function ($scope, $uibModalInstance, Wallet, wallet) {
							...
							Wallet.getRequired(wallet.address, function (e, confirmations) {
							...
							Wallet.getOwners(wallet.address, function (e, owners) {
								...
								$scope.data.owners = owners.map(function (address) {
									return Web3Service.toChecksumAddress(address)
								})
							}).call()
				...
				$scope.updateParams = function () {
					...
					// Get owners
					batch.add(
						Wallet.getOwners($scope.wallet.address, function (e, owners) {
							...
							if (!$scope.wallet.owners) {
								...
								$scope.owners.forEach(function (item, index) {
									...
									if (index == $scope.owners.length - 1) {
										// last item
					...
					// Get ETH Balance
					batch.add(
						Wallet.getBalance($scope.wallet.address, function (e, balance) {
							if (!e && balance) {
					...
					batch.add(
						Wallet.getRequired($scope.wallet.address, function (e, confirmations) {
					...
					// Get # required confirmations
					batch.add(
						Wallet.getRequired($scope.wallet.address, function (e, required) {
							if (required) {
								$scope.$apply(function () {
					...
					if ($scope.wallet.tokens) {
						Object.keys($scope.wallet.tokens).map(function (token) {
							// Get multisig balance
							batch.add(
								Token.balanceOf(token, $scope.wallet.address, function (e, balance) {
							...
							// Get account balance
							batch.add(
								Token.balanceOf(token, Web3Service.coinbase, function (e, balance) {
				...
				$scope.getParam = function (tx) {
						...
						switch (method) {
							...
							case "e20056e6":
								...
								return {
									title:
										"Replace owner " +
										$filter("addressCanBeOwner")(oldOwner, $scope.wallet) +
										" with " +
										$filter("addressCanBeOwner")(newOwner, $scope.wallet),
								}
							default:
								// Check abis in cache
								var abis = ABI.get()
					...
					// Check if trasanction was sent to an item in address book
						...
						return {
							title:
								"Transfer " + $filter("ether")(tx.value) + " to " + $scope.addressBook[tx.to].name,
						}
					} else {
						return {
							title:
								"Transfer " +
								$filter("ether")(tx.value) +
								" to " +
								$filter("addressCanBeOwner")(tx.to, $scope.wallet),
						}
					}
				}
				...
				$scope.getDestination = function (tx) {
					if (Wallet.wallets[tx.to]) {
						...
						$scope.wallet.owners &&
						$scope.wallet.owners[tx.to] &&
						$scope.wallet.owners[tx.to].name
					) {
					...
					// Destination is a Token
						...
						$scope.wallet.tokens &&
						$scope.wallet.tokens[tx.to] &&
						$scope.wallet.tokens[tx.to].name
					) {
							...
							ids.map(function (tx) {
									...
									Wallet.getTransaction($scope.wallet.address, tx, function (e, info) {
										if (!e && info.to) {
											...
											$scope.$apply(function () {
												...
												if (
													!$scope.transactions[tx].dataDecoded ||
													$scope.transactions[tx].dataDecoded.notDecoded ||
													($scope.transactions[tx].dataDecoded.usedABI &&
														(!savedABI || savedABI.abi))
												) {
													$scope.transactions[tx].dataDecoded = $scope.getParam(
														$scope.transactions[tx]
													)
												}
												// If destionation type has not been set
												if (!$scope.transactions[tx].destination) {
													$scope.transactions[tx].destination = $scope.getDestination(
														$scope.transactions[tx]
													)
												}
											})
								...
								txBatch.add(
									Wallet.getConfirmations($scope.wallet.address, tx, function (e, confirmations) {
										$scope.$apply(function () {
											$scope.transactions[tx].confirmations = Web3Service.toChecksumAddress(
												confirmations
											)
											// If the current user is among the array of confirmations, we can set the transaction
											// as confirmed by that user
											if (
												$scope.transactions[tx].confirmations.indexOf(Web3Service.coinbase) != -1
											) {
