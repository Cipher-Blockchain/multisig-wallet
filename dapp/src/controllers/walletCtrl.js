			...
			function (
				...
				$scope.updateParams = function () {
					...
					if ($scope.wallets) {
						...
						Object.keys($scope.wallets).map(function (address) {
							$scope.batch.add(
								Wallet.getBalance(address, function (e, balance) {
							...
							$scope.batch.add(
								Wallet.getRequired(address, function (e, confirmations) {
							 ...
							 */
							$scope.batch.add(
								Wallet.getOwners(address, function (e, owners) {
							...
							$scope.batch.add(
								Wallet.getLimit(address, function (e, limit) {
							...
							$scope.batch.add(
								Wallet.calcMaxWithdraw(address, function (e, max) {
				...
				function startup() {
					...
					if (
						...
						(!$scope.wallets || (!Object.keys($scope.wallets).length && !$rootScope.alreadyLogged))
					) {
