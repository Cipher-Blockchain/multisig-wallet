		...
		.controller(
			...
			function (
						...
						controller: function ($scope, $uibModalInstance) {
							...
							$scope.ok = function (option) {
								...
								Web3Service.webInitialized().then(function () {
									...
									CommunicationBus.startInterval(
										"updateInfo",
										txDefault.accountsChecker.checkInterval
									)
								})
							}
						},
				...
				$scope.updateInfo = function () {
					 ...
					 */
					Transaction.getEthereumChain().then(function (data) {
					...
					// init params
					$scope.paramsPromise = Wallet.initParams().then(
						function () {
							$scope.loggedIn = !isElectron
								? Web3Service.coinbase !== undefined && Web3Service.coinbase !== null
								: true
				 ...
				 */
				$scope.statusIcon = $sce.trustAsHtml(
					"<i class='fa fa-refresh fa-spin fa-fw' aria-hidden='true'></i>"
				)

				$scope.updateConnectionStatus = function () {
					$scope.connectionStatus = Connection.isConnected
					$scope.statusIcon = Connection.isConnected
						? $sce.trustAsHtml(" <i class='fa fa-circle online-status' aria-hidden='true'></i>")
						: $sce.trustAsHtml(
								"<i class='fa fa-refresh fa-spin fa-fw' aria-hidden='true'></i> Offline <i class='fa fa-circle offline-status' aria-hidden='true'></i>"
						  )
				}
				...
				function startup() {
					...
					var updateConnStatusIdx = CommunicationBus.addFn(
					...
					Web3Service.webInitialized().then(
						function () {
							...
							CommunicationBus.startInterval(
								updateConnStatusIdx,
								txDefault.connectionChecker.checkInterval
							)
							$scope.updateConnectionStatus()
							...
							$scope.updateInfo().then(function () {
								...
								CommunicationBus.startInterval(
									checkReceiptsIdx,
									txDefault.transactionChecker.checkInterval
								)
								...
								if (gdprTermsAccepted && !chooseWeb3ProviderShown && isElectron) {
									...
									txDefault.wallet !== "ledger" &&
									txDefault.wallet !== "lightwallet"
								) {
